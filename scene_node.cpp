...
void scene_node::execute(XMMATRIX * world, XMMATRIX * view, XMMATRIX * projection, XMFLOAT4 AmbC, XMVECTOR DirV, XMFLOAT4 DirC, XMVECTOR PLightV, XMFLOAT4 PLightC, XMVECTOR SpotV, XMFLOAT4 SpotC, XMVECTOR SpotD)
{
	...
	if (m_p_model) 
	{
		m_p_model->Draw(&local_world, view, projection, AmbC, DirV, DirC, PLightV, PLightC, SpotV, SpotC, SpotD);
	}
	for(int i = 0; i<m_children.size(); i++)
	{
		m_children[i]->execute(&local_world, view, projection, AmbC, DirV, DirC, PLightV, PLightC, SpotV, SpotC, SpotD);
	}
}
	...
	if(m_p_model)
	{
		...
		if((dx*dx + dy * dy + dz * dz) < pow((this->m_p_model->GetBoundingSphereRadius()* this->m_world_scale)+sqrt(ray_dir->x*ray_dir->x + ray_dir->y * ray_dir->y + ray_dir->z*ray_dir->z),2))
		{
			for(int i=0; i<m_p_model->GetObject()->numverts; i += 3)
			{
				...
				maths::Plane plane{};
				plane= mhelper->planeEquation(&p1, &p2, &p3);

				float v1= mhelper->planeTest(&plane, ray_dir);
				float v2 = mhelper->planeTest(&plane, ray_pos);
				if (mhelper->sign(v1) == mhelper->sign(v2))
				{
					XMVECTOR iPoint = mhelper->planeIntersection(&plane, ray_pos, ray_dir);
					if(mhelper->in_triangle(&p1, &p2, &p3, &iPoint))
					return true;
				}
			}
		}
...
bool scene_node::MoveForward(float d, scene_node* root_node, float dt)
{
	...
	m_pz += cos(m_yAngle * (XM_PI / 180.0))*d*dt;
	m_px += sin(m_yAngle * (XM_PI / 180.0))*d*dt;
