...
HRESULT Reflective::LoadObjModel(char* filename) {
	...
	constant_buffer_desc.ByteWidth = 240;
...
void Reflective::Draw(XMMATRIX* view, XMMATRIX* projection, XMFLOAT4 AmbC, XMVECTOR DirV, XMFLOAT4 DirC, XMVECTOR PLightV, XMFLOAT4 PLightC, XMVECTOR SpotV, XMFLOAT4 SpotC, XMVECTOR SpotD) {
	XMMATRIX world;
	...
	light_cb.PointLightRange = (1/15);
	light_cb.PointLightPos = PLightV;
	light_cb.PointLightC = PLightC;
	light_cb.SpotLightPos = SpotV;
	light_cb.SpotLightC = SpotC;
	light_cb.SpotLightDir = SpotD;
	light_cb.SpotOuterCone = cos(45 * (XM_PI / 180));
	light_cb.SpotInnerCone = cos(15 * (XM_PI / 180));
	light_cb.SpotRange = (1/15);
